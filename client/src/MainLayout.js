...
// --- Icon Components (for brevity, these are kept the same) ---
...
const ServerStatus = () => {
    ...
    useEffect(() => {
        const checkStatus = async () => {
            try { await axios.get('/api/health'); setStatus('ok'); } catch (error) { setStatus('down'); }
        };
        checkStatus();
        const interval = setInterval(checkStatus, 30000);
        return () => clearInterval(interval);
    }, []);
    const statusConfig = { ok: { color: 'bg-green-500', tooltip: 'Server is online' }, checking: { color: 'bg-yellow-500', tooltip: 'Checking...' }, down: { color: 'bg-red-500', tooltip: 'Server is offline' } };
    return (<div className="relative group flex items-center"><span className={`w-3 h-3 rounded-full ${statusConfig[status].color}`}></span><span className="absolute bottom-full mb-2 right-0 w-max px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100">{statusConfig[status].tooltip}</span></div>);
};
...
const MainLayout = ({ user }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col lg:flex-row font-sans">
            ...
            {/* ✅ FIX: Changed z-20 to z-40 to ensure the menu appears on top of the header */}
            <aside className={`fixed lg:relative inset-y-0 left-0 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out bg-[#4285F4] text-white w-64 lg:w-1/5 lg:m-4 lg:rounded-3xl p-8 flex flex-col justify-between z-40`}>
                <div>
                        ...
                        <ul>
                            {navItems.map(item => (
                                <li key={item.path} className="mb-6">
                                    <Link to={item.path} onClick={() => setIsMenuOpen(false)} className={`flex items-center p-2 rounded-lg hover:bg-white/20 ${location.pathname.startsWith(item.path) ? 'font-bold' : ''}`}>
                                        <item.icon />
                ...
                <div>
                    ...
                    <button onClick={handleLogout} className="flex items-center p-2 rounded-lg hover:bg-white/20 text-sm w-full">
                        <LogoutIcon />
            ...
            <div className="flex-1 flex flex-col">
                <header className="hidden lg:flex justify-between items-center p-8">
                    ...
                    <div className="flex items-center space-x-4">
                        <ServerStatus />
                        {user?.photo && <img src={user.photo} alt={user.displayName} className="rounded-full w-10 h-10" />}
